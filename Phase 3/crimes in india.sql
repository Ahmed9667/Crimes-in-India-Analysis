...
select IPC_Crimes.STATE , IPC_Crimes.DISTRICT , IPC_Crimes.years , MURDER from IPC_Crimes
 join (select STATE,years  , max(MURDER) as max_murder from IPC_Crimes
Group by STATE , years
order by years)s
on s.STATE = IPC_Crimes.STATE
and s.max_murder = IPC_Crimes.MURDER and s.years = IPC_Crimes.years ;
